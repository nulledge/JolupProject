...
config.task = 'train'
config.shuffle = True
config.annotation.path = 'D:/gesture/dataset/MPII/mpii_human_pose_v1_u12_2/mpii_human_pose_v1_u12_1.mat'
config.images.path = 'D:/gesture/dataset/MPII/images/'
config.log.path = 'log/'
config.pretrained.use = False
config.pretrained.path = 'pretrained/'

data = MPII(config.n_batch, config.task, config.shuffle, config.annotation.path, config.images.path)
...
with tf.Session() as sess:
    ...
    with tf.variable_scope('loss'):
        loss = tf.losses.mean_squared_error(
            tf.reshape(model.heatmaps_groundtruth, shape=[-1, 64 * 64, model.joints]),
            tf.reshape(model.heatmaps[0], shape=[-1, 64 * 64, model.joints]))

        for stage in range(1, model.stages):
            loss = loss + tf.losses.mean_squared_error(
                tf.reshape(model.heatmaps_groundtruth, shape=[-1, 64 * 64, model.joints]),
                tf.reshape(model.heatmaps[stage], shape=[-1, 64 * 64, model.joints]))

        tf.summary.scalar('loss', loss)

    # Loss and visualized predictions are collected.
    summary_merged = tf.summary.merge_all()

    # For batch normalization.
    update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_ops):
        optimizer = tf.train.RMSPropOptimizer(learning_rate=config.learning_rate).minimize(loss, global_step=global_step)

    if not os.path.exists(config.log.path):
        os.makedirs(config.log.path)
    writer = tf.summary.FileWriter(config.log.path, sess.graph)

    saver = tf.train.Saver(max_to_keep=20)
    if config.pretrained.use:
        list_of_files = glob.glob(os.path.join(config.pretrained.path, '*'))
        latest_file = max(list_of_files, key=os.path.getctime)
        file_name = os.path.basename(latest_file).split('.save')[0]
        saver.restore(sess, os.path.join(config.pretrained.path, '%s.save' % file_name))
    else:
        sess.run(tf.global_variables_initializer())

    tf.train.global_step(sess, global_step)
    for epoch in range(1, config.n_epoch + 1):
        with tqdm(total=len(data.image_set), unit=' images') as progress:
            progress.set_description("epoch: %d/%d" % (epoch, config.n_epoch))
            for train_images, train_heatmaps, _, _, _ in data:
                ...
                progress.update(train_images.shape[0])
